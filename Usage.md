...
After [installation](/INSTALL.md), there are three basic ways to invoke CodiumAI MergeMate:
...
- The different tools and sub-tools used by CodiumAI MergeMate are adjustable via the **[configuration file](mergemate/settings/configuration.toml)**.
In addition to general configuration options, each tool has its own configurations. For example, the `review` tool will use parameters from the [pr_reviewer](/mergemate/settings/configuration.toml#L16) section in the configuration file.
...
To ignore files or directories, edit the **[ignore.toml](/mergemate/settings/ignore.toml)** configuration file. This setting also exposes the following environment variables:
...
#### git provider
The [git_provider](mergemate/settings/configuration.toml#L4) field in the configuration file determines the GIT provider that will be used by MergeMate. Currently, the following providers are supported:
`
...
`<pr_url>` is the url of the relevant PR (for example: https://github.com/khulnasoft/mergemate/pull/50).
...
```
Any configuration value in [configuration file](mergemate/settings/configuration.toml) file can be similarly edited. Comment `/config` to see the list of available configurations.


### Working with GitHub App
When running MergeMate from GitHub App, the default [configuration file](mergemate/settings/configuration.toml) from a pre-built docker will be initially loaded.
...
The [github_app](mergemate/settings/configuration.toml#L108) section defines GitHub app specific configurations.  
...
You can override the default tool parameters by uploading a local configuration file called `.mergemate.toml` to the root of your repo.
...
```
When a new PR is opened, MergeMate will run the `describe` tool with the above parameters.
...
```
This means that when new code is pushed to the PR, the MergeMate will run the `describe` and incremental `review` tools.  
...
#### Editing the prompts
The prompts for the various MergeMate tools are defined in the `mergemate/settings` folder.
In practice, the prompts are loaded and stored as a standard setting object.
Hence, editing them is similar to editing any other configuration value - just place the relevant key in `.mergemate.toml`file, and override the default value.

For example, if you want to edit the prompts of the [describe](./mergemate/settings/pr_description_prompts.toml) tool, you can add the following to your `.mergemate.toml` file:
```
...
"""
```
Note that the new prompt will need to generate an output compatible with the relevant [post-process function](./mergemate/tools/pr_description.py#L137).

### Working with GitHub Action
`GitHub Action` is a different way to trigger MergeMate tools, and uses a different configuration mechanism than `GitHub App`.
...
Note that you can give additional config parameters by adding environment variables to `.github/workflows/mergemate.yml`, or by using a `.mergemate.toml` file in the root of your repo, similar to the GitHub App usage.

For example, you can set an environment variable: `pr_description.add_original_user_description=false`, or add a `.mergemate.toml` file with the following content:
```
[pr_description]
add_original_user_description = false
```
...
Similar to GitHub app, when running MergeMate from BitBucket App, the default [configuration file](mergemate/settings/configuration.toml) from a pre-built docker will be initially loaded.
...
You can configure in your local `.mergemate.toml` file which tools will **run automatically** when a new PR is opened.

Specifically, set the following values:
```yaml
[bitbucket_app]
auto_review = true    # set as config var in .mergemate.toml
auto_describe = true  # set as config var in .mergemate.toml
auto_improve = true   # set as config var in .mergemate.toml
```
...
#### Extra instructions
All MergeMate tools have a parameter called `extra_instructions`, that enables to add free-text extra instructions. Example usage:
```
...
See [here](mergemate/algo/__init__.py) for the list of available models.
To use a different model than the default (GPT-4), you need to edit [configuration file](mergemate/settings/configuration.toml#L2).
...
Also review the [AiHandler](mergemate/algo/ai_handler.py) file for instruction how to set keys for other models.
...
For the `review`, `describe`, `ask` and `add_docs` tools, if the token budget allows, MergeMate tries to increase the number of lines of context, via the parameter:
```
[config]
patch_extra_lines=3
```
...
If the PR is too large (see [PR Compression strategy](./PR_COMPRESSION.md)), MergeMate automatically sets this number to 0, using the original git patch.
