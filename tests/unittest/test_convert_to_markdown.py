...
from mergemate.algo.utils import convert_to_markdown
...
class TestConvertToMarkdown:
    ...
    def test_simple_dictionary_input(self):
        input_data = {'review': {
            ...
            {'relevant_file': '``mergemate/git_providers/git_provider.py\n``', 'language': 'python\n',
...
class TestBR:
    def test_br1(self):
        file_change_description = '- Imported `FilePatchInfo` and `EDIT_TYPE` from `mergemate.algo.types` instead of `mergemate.git_providers.git_provider`.'
        ...
        expected_output = ('<li>Imported <code>FilePatchInfo</code> and <code>EDIT_TYPE</code> from '
                           '<code>mergemate.algo.types</code> instead <br>of '
                           '<code>mergemate.git_providers.git_provider</code>.')
