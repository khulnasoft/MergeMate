...
class GithubProvider(GitProvider):
    ...
    def get_repo_settings(self):
        try:
            # contents = self.repo_obj.get_contents(".mergemate.toml", ref=self.pr.head.sha).decoded_content

            # more logical to take 'mergemate.toml' from the default branch
            contents = self.repo_obj.get_contents(".mergemate.toml").decoded_content
            return contents
        except Exception:
            return ""
        ...
        if deployment_type == 'user':
            try:
                ...
                raise ValueError(
                    ...
                    "https://github.com/KhulnaSoft/mergemate#method-2-run-from-source") from e
