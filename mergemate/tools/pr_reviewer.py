...
from mergemate.algo.utils import convert_to_markdown, load_yaml, try_fix_yaml, set_custom_labels, get_user_labels, \
    ModelType
from mergemate.config_loader import get_settings
from mergemate.git_providers import get_git_provider
from mergemate.git_providers.git_provider import IncrementalPR, get_main_pr_language
...
class PRReviewer:
    ...
    def _prepare_pr_review(self) -> str:
        ...
        if self.incremental.is_incremental:
            ...
            data.update({'Incremental PR Review': {
                "⏮️ Review for commits since previous MergeMate review": incremental_review_markdown_text}})
    ...
    def auto_approve_logic(self):
        ...
        if get_settings().pr_reviewer.enable_auto_approval:
            ...
            self.git_provider.publish_comment("Auto-approval option for MergeMate is disabled. "
                                              "You can enable it via a [configuration file](https://github.com/KhulnaSoft/mergemate/blob/main/docs/REVIEW.md#auto-approval-1)")